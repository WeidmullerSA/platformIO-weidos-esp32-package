...
////////////////////////////////////////////////////////////////////////////////////////////////////
void HardwareSerialPIC::begin(unsigned long baud, uint32_t config, int8_t rxPin, int8_t txPin, bool invert, unsigned long timeout_ms, uint8_t rxfifo_full_thrhd) 
{
...
int HardwareSerialPIC::available() {
  ...
  do {
      do {
	        ...
	        int req_resp = Wire.requestFrom((int) i2cAddr, (int) 2);
...
int HardwareSerialPIC::availableForWrite() {
	Wire.beginTransmission(i2cAddr);
  	...
  	Wire.requestFrom((int) i2cAddr, (int) 2);
  	if (Wire.available() != 2) {
    	return 0;
  	}
...
int HardwareSerialPIC::read() {
  ...
  if(numberReadBytes > 128)
    numberReadBytes = 128;	
	
  Wire.beginTransmission(i2cAddr);
  Wire.write(FIFO_RX_REGISTER);
  Wire.endTransmission();
  int test1 = Wire.requestFrom((int) i2cAddr, (int) numberReadBytes);
...
int HardwareSerialPIC::readAndSave() {
  ...
  Wire.requestFrom((int) i2cAddr, (int) numberReadBytes);
  if (Wire.available() != numberReadBytes) {	
    return -1;
  }
...
size_t HardwareSerialPIC::read(uint8_t* buff, size_t size) {
  ...
  Wire.requestFrom((int) i2cAddr, (int) size);
